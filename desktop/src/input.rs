// Adapted from GPUI 0.2.2 examples/input.rs, Apache-2.0. See ../NOTICE.
use std::ops::Range;
...
impl TextInput {
    pub fn new(placeholder: &str, password: bool, cx: &mut Context<Self>) -> Self {
        Self {
    ...
    fn display_offset(&self, offset: usize) -> usize {
        if self.password {
            self.content[..offset].chars().count() * 3
        } else {
            offset
        }
    }
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), input.colors.2)
        } else {
            (
                if input.password {
        ...
        let runs = if let Some(marked_range) = input.marked_range.as_ref() {
            let marked_range =
                input.display_offset(marked_range.start)..input.display_offset(marked_range.end);
            vec![
                TextRun {
