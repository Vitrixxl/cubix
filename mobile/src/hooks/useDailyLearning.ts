...
import { dailyAssignment, EMPTY_LEARNING_PLAN, isLearningTrack, learningCases, learningKey, learningStatus, localDay, type LearningMode, type LearningPlan } from "../../../src/client/lib/dailyLearning";
...
export function useDailyLearning() {
  ...
  const mode: LearningMode = puzzle === "333" && isLearningTrack(plan.mode) ? plan.mode : "practice";
  ...
  return { mode, assignment, status: learningStatus(pool, learned, assignment), setMode: (mode: LearningMode) => save({ ...plan, mode }) };
}
