...
import { EDGE_STROKE, ISO_VIEWBOX, SEAM_FILL, TILE_STROKE, TOP_VIEWBOX, isoCells, isoEdges, isoHull, topLayerCells, viewForMask, type CubeMask, type DiagramView } from "../../../src/shared/cubeDiagram";
...
export const StaticCubeSvg = memo(function StaticCubeSvg({ state, size = 110, mask = "full", view = viewForMask(mask) }: { state: CubeState; size?: number; mask?: CubeMask; view?: DiagramView }) {
  if (view !== "iso") return <Svg width={size} height={size} viewBox={TOP_VIEWBOX}>
    <G {...TILE_STROKE}>
