...
export function SolveInfoButton({ solve }: { solve: SolveDto }) {
  ...
  return <>
    ...
    {position && createPortal(<div data-placement={position.above ? "top" : "bottom"} id={id} ref={popover} className="solve-details-popover popover" role="region" aria-label="Solve details" style={{ left: position.left, top: position.top, maxHeight: position.above ? position.top - 8 : window.innerHeight - position.top - 8 }}>
      <span className="label">{contextLabel(solve)}</span>
      <time dateTime={solve.created_at}>{new Date(solve.created_at).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}</time>
