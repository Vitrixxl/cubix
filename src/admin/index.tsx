...
function Admin() {
 ...
 {tab==="requests"&&<><div className="panel-heading"><div><h2>Journal des requêtes</h2><p>200 dernières correspondances · {number(traffic.retained)} / {number(traffic.capacity)} requêtes en mémoire</p></div><span className="badge">HTTP + ouvertures WebSocket</span></div><div className="filters"><input aria-label="Filtrer les requêtes par IP" placeholder="Adresse IP…" value={ip} onChange={e=>{setIp(e.target.value);setIpPage(0)}}/><input aria-label="Filtrer par route" placeholder="Route /api/…" value={path} onChange={e=>setPath(e.target.value)}/><select aria-label="Filtrer par statut" value={status || "all"} onChange={e=>setStatus(e.target.value === "all" ? "" : e.target.value)}>{[["all","Tous les statuts"],["2","2xx · Succès"],["3","3xx · Redirections"],["4","4xx · Erreurs client"],["5","5xx · Erreurs serveur"],["429","429 · Rate limit"]].map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div><div className="table-scroll"><table><thead><tr><th>Heure</th><th>IP</th><th>Méthode</th><th>Route</th><th>Statut</th><th>Durée</th></tr></thead><tbody>{traffic.requests.map(row=><tr key={row.id}><td>{time(row.at)}</td><td><button className="text-button" onClick={()=>setIp(row.ip)}>{row.ip}</button></td><td><code>{row.method}</code></td><td className="route" title={row.path}>{row.path}</td><td><span className={`status ${row.status>=400?"bad":"good"}`}>{row.status}</span></td><td>{row.durationMs.toFixed(1)} ms</td></tr>)}</tbody></table>{!traffic.requests.length&&<p className="empty">Aucune requête ne correspond aux filtres.</p>}</div></>}
