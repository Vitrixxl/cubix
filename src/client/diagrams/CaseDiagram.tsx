...
import {viewForStage} from '../../shared/cubeDiagram';
...
export const CaseDiagram=memo(function CaseDiagram({c,size=102}:{c:CaseDto;size?:number}){
  return c.diagram ? <img className="case-diagram" src={c.diagram} width={size} height={size} alt={`${c.name} setup`} loading="lazy" draggable={false}/> : <StaticCubeSvg state={caseState(c)} size={size} mask={maskForStage(c.stage)} view={viewForStage(c.stage)}/>;
});
